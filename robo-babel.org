...
<<<<<<< HEAD
  ...
  (defun tr/generate-code-link ()
    ...
    (let* ((path (buffer-file-name))
           ...
           (seen ()))
      (while (progn
               (setq seen (push (car (last list)) seen))
               (setq list (butlast list))
               (not (-filter (lambda (dir) (string= dir ".git"))
                        (directory-files (mapconcat 'identity list "/"))))))
      (format "https://code.amazon.com/packages/%s/blobs/heads/%s/--/%s#line-%d"
              (car (last list)) (tr/git-get-branch) (mapconcat 'identity seen "/") (line-number-at-pos))))
  
  (defun tr/open-code-amazon-com ()
    (interactive)
    (browse-url (tr/generate-code-link)))
  
  (global-set-key "\C-xlC" 'tr/open-code-amazon-com)
#+END_SRC  
*** Defaults
#+BEGIN_SRC emacs-lisp
