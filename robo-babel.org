...
#+BEGIN_SRC emacs-lisp
  (define-derived-mode git-projects-mode special-mode "Git Overview")
  
  ;(define-key git-projects-mode-map (kbd "RET") 'gs-git-proj/open-in-magit)
  ;(define-key git-projects-mode-map (kbd "n") 'gs-git-proj/next-project)
  ;(define-key git-projects-mode-map (kbd "p") 'gs-git-proj/prev-project)
  ;(define-key git-projects-mode-map (kbd "g") 'gs-git-proj/rebuild-buffer)
  
  (defface project-header
  ...
  (defun tr-project-display/present-title (title)
    (concat "\n" (propertize (concat title "\n")
                             'face 'project-header)))
  (defun tr-project-display/present-body (body)
    (propertize body 'face 'project-body))
  
  (defun tr/list-projects ()
    "Loops through all known projects from projectile and applies git-tldr to them"
    (interactive)
    (let ((bufname (get-buffer-create "*Projects*")))
          ...
          (mapc
           (lambda (project)
             (let ((default-directory project))
               (insert (tr-project-display/present-title project))
               (insert (tr-project-display/present-body (shell-command-to-string "git tldr")))))
           (projectile-relevant-known-projects))
          (git-projects-mode)
          (pop-to-buffer bufname)))))
  
  (global-set-key "\C-ct" 'tr/list-projects)
#+END_SRC
